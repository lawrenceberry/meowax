...
def _run_julia_moving_diffusion_spike(
    benchmark,
    solver_factory,
    moving_diffusion_spike_system,
    ensemble_size,
    ensemble_backend,
):
...
@pytest.mark.parametrize("ensemble_size", _ENSEMBLE_SIZES)
def test_diffrax_kencarp5(benchmark, moving_diffusion_spike_system, ensemble_size):
...
@pytest.mark.parametrize("ensemble_size", _ENSEMBLE_SIZES)
def test_diffrax_kvaerno5(benchmark, moving_diffusion_spike_system, ensemble_size):
...
def test_julia_tsit5(
    benchmark, moving_diffusion_spike_system, ensemble_size, ensemble_backend
):
    """Julia Tsit5 benchmark with mass-conservation validation."""
    system, results_np = _run_julia_moving_diffusion_spike(
        benchmark,
