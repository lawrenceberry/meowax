"""Rodas5 solver — single-loop batched variant for linear ODEs.

Solves dy/dt = J(t, params) @ y where the Jacobian is supplied directly
via ``jac_fn(t, params) -> [n_vars, n_vars]``.  The Jacobian depends on
time and static parameters but NOT on the current state y.
...
def make_solver(
    ...
    *,
    ...
    jac_fn : callable
        ...
        The Jacobian may depend on time and static parameters but must not
        depend on the current state *y*.
    ...
    matvec_dtype = (
        jnp.float32 if linear_solver_precision == "fp32" else jnp.float64
    )
    dot_precision = (
    ...
    _jac_fn_batched = jax.vmap(jac_fn, in_axes=(0, 0))
